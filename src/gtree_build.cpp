/*
 * this code is copied from the repo.
 * it is too slow and does not match the dataformat given. 
 * modification and parallelization are applied
 */
...
void hierarchy_shortest_path_calculation(){
	...
	for ( int i = treenodelevel.size() - 1; i >= 0; i-- ){
		printf("Hierarchy %d total %lu\n", i, treenodelevel[i].size());
		for ( int j = 0; j < treenodelevel[i].size(); j++ ){
			if (clock() - tc > 1000000) {
				tc = clock();
				printf("H%d %d / %lu\n", i, j, treenodelevel[i].size());
			}
